  ...
  switch (action.type) {
    ...
    case actions.PUSH: {
      ...
      if (currentState.modal) {
        ...
        return setActiveKey(navState, action.key);
      }
      ...
      let combinedState = Object.assign({}, currentState, { containerState: newcontainerState });
      return setActiveKey(combinedState, action.key);
    }
    case actions.MODAL: {
      let scene = findScene(action.key);
      let props = Object.assign({}, scene, { ...action.props });
      let modalState = NavigationStateUtils.push(currentState.modalState, { key: action.key, props });
      let navState = Object.assign({}, currentState, { modalState, modal: true });
      return setActiveKey(navState, action.key);
    }
    ...
    case actions.POP: {
      if (!currentState.modal) {
        ...
        let combinedState = Object.assign({}, currentState, { containerState: newcontainerState });
        return setActiveKey(combinedState, activeKey);
      }
      ...
      let combinedState = Object.assign({}, navState, { modal: navState.modalState.children.length > 0 });
      return setActiveKey(combinedState, activeKey);
    }
    case actions.POP_TO: {
      if (!currentState.modal) {
      ...
      console.warn('You called popTo within a modal. This is not supported.'); // eslint-disable-line no-console
      return currentState;
    }
