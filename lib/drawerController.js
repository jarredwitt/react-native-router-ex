...
class DrawerController extends Component {
  ...
  componentWillMount() {
    this.contentOffset = {
      x: this.props.leftMenuWidth,
      y: 0,
    };
  }
  componentDidUpdate() {
    if (this.props.navState.leftDrawerVisible) {
      this.refs.scrollView.scrollTo({ x: 0 });
    } else if (this.props.navState.rightDrawerVisible) {
      this.refs.scrollView.scrollTo({ x: this.contentOffset.x + this.props.rightMenuWidth });
    } else {
      this.refs.scrollView.scrollTo({ x: this.props.leftMenuWidth });
    }
  }
  render() {
