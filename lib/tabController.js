...
class TabController extends Component {
  ...
  _renderNavigation = (navigationState) => {
    ...
    if (currentChild.props.type === 'SCENE') {
      ...
      scene = (
        <View style={{ flex: 1 }}>
          <SceneComponent {...otherProps} navigate={navigationHelper(this.props.dispatch)} />
        </View>
      );
    }
        ...
        {this._tabs.map((tab, index) => {
          ...
          return (
            <View key={index} style={style}>
              <TabView navState={navigationState.tabState[index]} scene={tab} scenes={this.props.scenes} dispatch={this.props.dispatch} />
            </View>
          );
        })}
